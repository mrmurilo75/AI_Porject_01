...
class Candidate extends ArrayList<Coordinate> {
	...
	private static SegmentMap pheromorn;
...
//	IntersectionList intersected;
	...
	public Candidate(Candidate parent){
		...
		if(Q != null && pheromorn != null){
			...
			cur = base.remove( (new Random()).nextInt(this.size()) );	// start at a random
			while (cur != null) {
				this.add(cur);
...
//		intersected = new IntersectionList(this);
	...
	*/
	 ...
	 * @param neighbours the points to which 'i' can link to form a segment
	 * @return a segment generated by ant.
	 */
	public Coordinate findAntPath(Coordinate i, ArrayList<Coordinate> neighbours){
		TreeMap<Double, Coordinate> probability = new TreeMap<Double, Coordinate>();
		double value = 0, last = 0;
		for(Coordinate k : neighbours)
			value += (pheromorn.get( new Pair<Coordinate>(i, k) ) * ( 1.0/euclidianDistance(i,k) ));
		for(Coordinate j : neighbours){
